const SectionHeading = ({ eyebrow, title, description, align = 'left', light = false, as: Tag = 'h2' }) => {
  ...
  return (
    <div className={wrapper}>
      {eyebrow && (
        <p className={`font-medium text-sm tracking-widest uppercase mb-3 ${light ? 'text-emerald-300' : 'text-moss'}`}>
          {eyebrow}
        </p>
      )}
      <Tag className={`text-3xl md:text-4xl font-semibold mb-3 ${light ? 'text-white' : 'text-forest'}`}>
        {title}
      </Tag>
      {description && (
